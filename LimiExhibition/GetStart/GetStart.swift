...
//        }
    ...
    private func createInstallerUser() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            // First try Codable decoding (more robust)
            do {
                ...
                if decoded.success, let token = decoded.data?.token, !token.isEmpty {
                    if let username = decoded.data?.data?.username, !username.isEmpty {
                        print("Installer username:", username)
                    }
                    AuthManager.shared.saveToken(token)
                    print("Token saved:", token)
                    DispatchQueue.main.async { navigateToAddDevice = true }
                    return
                }
                // If decoding succeeded but fields not present as expected, fall through to raw logging
            } catch {
                print("Codable decode error:", error.localizedDescription)
            }

            // Fallback: log raw response and attempt loose JSON parsing for diagnostics
    ...
    private func createInstallerUser() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                print("Decoded success:", decoded.success)
                if decoded.success, let token = decoded.data?.token, !token.isEmpty {
                    if let username = decoded.data?.data?.username, !username.isEmpty { print("Installer username:", username) }
                    AuthManager.shared.saveToken(token)
                    print("Token saved:", token)
                    DispatchQueue.main.async { navigateToAddDevice = true }
                    return
