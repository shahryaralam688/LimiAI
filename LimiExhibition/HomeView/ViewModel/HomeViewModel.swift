...
class HomeViewModel: ObservableObject {
    ...
    @Published var showModulerView = false
    @Published var showConnectedDevices = false
    @Published var showConfigurator = false
    @Published var showARView = false
    @Published var showRoomScan = false
    ...
    @Published var showDemoAddingWifi = false
    ...
    private func sendDeviceToBackend(device: WifiDevice) {
        guard let token = authProvider.token(), !token.isEmpty else { return }
        ...
        networkPerformer.perform(request)
    }
}
