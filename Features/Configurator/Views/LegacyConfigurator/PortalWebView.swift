...
struct PortalWebView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                LimiModuleSubtitle(text: "Explore and customize in AR")
                tabPicker
                    .padding(.top, 12)
...
extension PortalWebView {
    private func checkLightConfigs() {
        LimiConfiguratorAPI.checkLightConfigs { result in
...
struct WebViewContainer: UIViewRepresentable {
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        private func fetchLightConfig(for spanID: String) {
            ...
            LimiConfiguratorAPI.fetchLightConfig(spanID: spanID) { json in
                guard let json else {
                ...
                guard !extractedDownloadId.isEmpty else {
                    DispatchQueue.main.async { self.isLoading.wrappedValue = false }
                    return
                }

                ConfiguratorDownloadService.downloadIfNeeded(snapId: extractedDownloadId) { _ in
