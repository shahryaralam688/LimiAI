...
private struct OrbFallbackView: View {
    let hue: Float
    ...
    var body: some View {
        Circle()
            .fill(
                RadialGradient(
                    colors: [
                        Color(hue: Double(hue), saturation: 0.6, brightness: 0.95),
                        Color(hue: Double(hue), saturation: 0.4, brightness: 0.5)
                    ],
                    center: .center,
                    startRadius: 20,
                    endRadius: 180
