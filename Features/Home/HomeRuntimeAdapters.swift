...
protocol HomeBluetoothControlling: ObservableObject {
    ...
    var storedHubs: [Hub] { get }
    var demoStoredHubs: [Hub] { get }
...
final class HomeBluetoothAdapter: HomeBluetoothControlling {
    ...
    @Published private(set) var storedHubs: [Hub] = []
    @Published private(set) var demoStoredHubs: [Hub] = []
    ...
    init(manager: BluetoothManager = .shared) {
        ...
        self.storedHubs = manager.storedHubs
        self.demoStoredHubs = manager.DemostoredHubs
        ...
        manager.$storedHubs
            .receive(on: DispatchQueue.main)
            .sink { [weak self] in self?.storedHubs = $0 }
            .store(in: &cancellables)

        manager.$DemostoredHubs
            .receive(on: DispatchQueue.main)
            .sink { [weak self] in self?.demoStoredHubs = $0 }
            .store(in: &cancellables)
    }
