...
struct EnhancedBottomNavigationView: View {
    ...
    init(
        showARScan: Binding<Bool>,
        showCustomer: Binding<Bool>,
        showGrouping: Binding<Bool>,
        ...
        self._showARScan = showARScan
        self._showCustomer = showCustomer
        self._showGrouping = showGrouping
    ...
    private var fabButton: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(isFABOpen ? "Close shortcuts menu" : "Open shortcuts menu")
        .scaleEffect(isFABOpen ? 0.95 : 1.0)
    }

    // MARK: - Blob Menu (organic blobs orbit around FAB, sweep left→top→right)
    // Voice ui_guide (ContextManager.homeShellUIGuide): layers = close menu, brain = voice chat, desktop = AR portal.

    private var blobMenuItems: some View {
        let neumorphBase = Color.appSurfaceTertiary
        let items: [(icon: String, accessibilityLabel: String, action: () -> Void)] = [
            ("square.3.layers.3d", "Close menu", {
                closeFAB()
            }),
            ("brain", "Voice chat", {
                closeFAB(); showVoiceView = true
            }),
            ("desktopcomputer", "AR portal", {
                closeFAB()
