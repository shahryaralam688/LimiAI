...
final class WebRTCVoiceClient: NSObject, ObservableObject {
    ...
    init(backendBaseURL: URL) {
        ...
        self.webhookURL = URL(string: APIConstants.webHook)
        super.init()
        setupFactory()
    ...
    private func sendTextBody(_ text: String) {
        ...
        do {
            ...
            guard var req = LimiHTTPClient.buildRequest(url: webhookURL, method: "POST", auth: .webhook) else {
                log("Webhook skipped: no auth token")
                return
            }
    ...
    private func fetchEphemeralKey(completion: @escaping (Result<String, Error>) -> Void) {
        guard let url = URL(string: APIConstants.limiAISession) else {
            log("❌ Invalid session URL: \(APIConstants.limiAISession)")
            completion(.failure(NSError(domain: "url", code: -1)))
            return
        }
        guard var request = LimiHTTPClient.buildRequest(url: url, method: "POST") else {
