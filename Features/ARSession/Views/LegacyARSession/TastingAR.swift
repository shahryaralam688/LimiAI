...
//            }
    ...
    private func attachPendant(index: Int, to root: Entity, coordinator: Coordinator) {
        ...
        guard let pendantURL = ConfiguratorModelStore.bundledModelURL(name: baseName) else {
            print("Could not find \(baseName).usdz in art.scnassets")
            return
        }

        guard let basePendant = ConfiguratorModelStore.loadEntity(from: pendantURL) else {
            print("Failed to load pendant model \(baseName).usdz")
            return
        }
    ...
    private static func bundledBaseName(for index: Int) -> String {
        switch index {
        case 1: return "Base1"
        case 2: return "Base2"
        case 3: return "Base3"
        default: return "Base1"
        }
    }
    ...
    func makeUIView(context: Context) -> ARView {
        ...
        let baseFileName = Self.bundledBaseName(for: selectedBaseIndex)

        guard let loadedEntity = ConfiguratorModelStore.loadEntity(downloadId: downloadId, bundledName: baseFileName) else {
            return arView
        }
        ...
        if let sourceURL = ConfiguratorModelStore.resolveModelURL(downloadId: downloadId, bundledName: baseFileName) {
    ...
    func updateUIView(_ uiView: ARView, context: Context) {
        ...
        if let modelEntity = context.coordinator.modelEntity {
            if context.coordinator.currentBaseIndex != selectedBaseIndex {
                ...
                guard let newLoadedEntity = ConfiguratorModelStore.loadEntity(
                    downloadId: downloadId,
                    bundledName: baseFileName
                ) else {
                    print("Failed to load model at \(baseFileName).usdz")
                    return
                }
