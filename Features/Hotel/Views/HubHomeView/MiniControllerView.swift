...
struct MiniControllerView: View {
    ...
    private func pwmOpacity(for ledIndex: Int?) -> Double {
        guard let ledIndex, ledIndex >= 1, ledIndex <= pwmBrightness.count else { return 0.3 }
        return pwmBrightness[ledIndex - 1] / 100.0
    }
                ...
                ZStack {
                    ...
                    Ellipse()
                        ...
                        .opacity(
                            ...
                            : pwmOpacity(for: selectedPWM)  // PWM mode opacity
                        )
                        .blur(radius: 15)
                        .padding(.top, 80)
...
//                            .padding(.horizontal, 20)
        ...
        .onChange(of: pwmBrightness) { _, newValues in
            guard newValues.count >= 5 else { return }
...
//            self.storeHistory.addElement(hub: self.hub, byteArray: byteArray)
    ...
    private func sendDeviceInfo(deviceInfo: String) {
        LimiDeviceAPI.postProcessDeviceData(deviceInfo: deviceInfo)
    }
}
